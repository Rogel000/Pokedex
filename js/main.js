document.addEventListener("DOMContentLoaded", () => {
  ...
  const capturePokemon = document.querySelector(".pokeball");
  ...
  if (capturePokemon) {
    capturePokemon.addEventListener("click", capturPokemon);
  } else {
    console.error("L'élément '.pokeball' n'a pas été trouvé.");
  }
        ...
        .then((pokemonData) => {
          ...
          myPokemonsDiv.innerHTML += `
              <div class="pokemon-card">
                ...
                <h2>${id} - ${name}</h2>
                <button class="delete-pokemon" data-id="${id}">Supprimer</button>
              </div>
            `;
        });
    });
